...
struct qnode nodes_array[P_MAXELM] = { 
		...
		{"{"  	   , P_LBRACKS  , {M_LBRACKS_DML, M_LBRACKS_DDL},	0},
		{"}"  	   , P_RBRACKS  , {M_RBRACKS_DML, M_RBRACKS_DDL},	0},
		...
		{""  	   , P_TABNAME , {M_TABNAME_DML, M_TABNAME_DDL},	0},
...
/*
Hash list that includes all the keywords
Each keyword is positioned based on its hash value.
Items with the same hash are linked together.
*/
...
int find_qnode(struct token *tk){
	...
	printf("Name of token is %s \n", tk->name);

	id = find_hash_node(tk->name);


	printf("After find_hash_node  \n");

	if( id == -ENULL)
		return -ENULL;


	hnode = &hash_node_list[id];
	...
	do{
		...
		if(!strcmp(hnode->qptr->name,tk->name))
		{

			printf("problem after str  \n");
